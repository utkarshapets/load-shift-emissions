...
def load_shift(
    emissions,
    load,
    time_series_differential,
    proportion_shiftable,
    shiftable_window_length,
):
    """
    ...
    """
    assert (
        len(emissions) == len(load) == len(time_series_differential)
    ), "time series must all be the same length"
    assert (
        0 <= proportion_shiftable <= 1
    ), "proportion_shiftable must be between 0 and 1"
    ...
    new_load_lower = np.array(load) - shiftable_load * np.array(
        [1 if i == peak_MOER_timestep else 0 for i in range(T)]
    )
    ...
    load_delta_times = [
        (
            1
            if (
                (i <= peak_MOER_timestep + shiftable_window_length)
    ...
    constraints = [
        new_load_lower <= new_load,
        new_load <= new_load_upper,
        cp.sum(new_load) == cp.sum(load),
    ]
...
def load_shift_penalty(
    ...
    load_delta_times = [
        (
            1 + 0.05 * (i - peak_MOER_timestep) / 4
            if (
                (i <= peak_MOER_timestep + shiftable_window_length)
                and (i >= peak_MOER_timestep)
            )
            else (
                1 + 0.05 * (peak_MOER_timestep - i) / 4
                if (
                    (i <= peak_MOER_timestep)
                    and (i >= peak_MOER_timestep - shiftable_window_length)
                )
                else 0
            )
    ...
    shift_penalty_times = [
        (
            1 + 0.05 * (i - peak_MOER_timestep) / 4
            if i > peak_MOER_timestep
            else (1 + 0.05 * (peak_MOER_timestep - i) / 4)
        )
        for i in range(T)
    ]
    ...
    load = np.array(load)
    constraints = [
        A @ new_load <= np.reshape([new_load_upper, -new_load_lower], (2 * len(load),)),
        penalty @ new_load == penalty @ load,
    ]
...
def load_shift_refrigerator(
    ...
    T = len(load)
    time_series_differential = utils.get_time_series_differential(
        emissions, shiftable_window_length
    )
...
def load_shed(emissions, load, proportion_shedable, shedable_window_length):
    ...
    rolling_sum = [
        sum(emissions[i : i + shedable_window_length])
        for i in range(len(emissions) - shedable_window_length)
    ]
    start_of_shed = np.argmax(rolling_sum)
    new_load = (
        list(load[:start_of_shed])
        + list(
            (1 - proportion_shedable)
            * np.array(load[start_of_shed : start_of_shed + shedable_window_length])
        )
        + list(load[start_of_shed + shedable_window_length :])
    )
